...
@dp.message_handler(text=buttons.add_queue, state=AdminStates.MAIN_MENU, user_id=settings.ADMINS_IDS)
async def start_from_registration(message: types.Message):
    await AdminSwitchers.send_queue_name(message)


@dp.message_handler(text=buttons.back, state=AdminStates.SEND_QUEUE_NAME, user_id=settings.ADMINS_IDS)
async def start_from_registration(message: types.Message):
    await AdminSwitchers.main_menu(message)


@dp.message_handler(content_types=["text"], state=AdminStates.SEND_QUEUE_NAME, user_id=settings.ADMINS_IDS)
async def start_from_registration(message: types.Message):
